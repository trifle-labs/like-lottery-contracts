...
async function main() {
  ...
  if (!prize) {
    ...
    console.error(
      'Usage: PRIZE=213.74 BEFORE_DATETIME="2024-01-15T12:00:00.000Z" GIVEAWAY_INDEX=0 npx hardhat run scripts/emit-snapshot-hash-v2.ts --network <network>'
    );
    process.exit(1);
  }
  ...
  if (!dateRegex.test(beforeDateTime)) {
    ...
    console.error(
      'Usage: PRIZE=1000 BEFORE_DATETIME="2024-01-15T12:00:00.000Z" GIVEAWAY_INDEX=0 npx hardhat run scripts/emit-snapshot-hash-v2.ts --network <network>'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const contract = LikeLotteryV2.attach(contractAddress);
